...
namespace Chess
{
    ...
    public class ChessGame
    {
        ...
        //new stuff
        int[] whiteSpaces = new int[8];
        int[] blackSpaces = new int[8];
        ...
        /// </summary>
        public ChessGame(bool chess960 = false)
        {
           ...
           Chess960Set(whiteSpaces);
           blackSpaces = Reverse(whiteSpaces);
           int index = 0;
...
//			Board = new Pieces.Piece?[8, 8]
//{
//				{ null, new Pieces.Pawn(new Point(0, 1), true), null, null, null, null, new Pieces.Pawn(new Point(0, 6), false), null },
//				{ null, new Pieces.Pawn(new Point(1, 1), true), null, null, null, null, new Pieces.Pawn(new Point(1, 6), false), null },
//				{ null, new Pieces.Pawn(new Point(2, 1), true), null, null, null, null, new Pieces.Pawn(new Point(2, 6), false), null },
//				{ null, new Pieces.Pawn(new Point(3, 1), true), null, null, null, null, new Pieces.Pawn(new Point(3, 6), false), null },
//				{ null, new Pieces.Pawn(new Point(4, 1), true), null, null, null, null, new Pieces.Pawn(new Point(4, 6), false), null },
//				{ null, new Pieces.Pawn(new Point(5, 1), true), null, null, null, null, new Pieces.Pawn(new Point(5, 6), false), null },
//				{ null, new Pieces.Pawn(new Point(6, 1), true), null, null, null, null, new Pieces.Pawn(new Point(6, 6), false), null },
//				{ null, new Pieces.Pawn(new Point(7, 1), true), null, null, null, null, new Pieces.Pawn(new Point(7, 6), false), null }
//};
            ...
            if(chess960)
            {
                ...
                {
                    Board[whiteSpaces[i], 0] = GetWhiteType(index, whiteSpaces[i]);
                    index++;
                }
			    ...
			    {
				    Board[blackSpaces[i], 7] = GetBlackType(index, blackSpaces[i]);
				    index++;
			    }
			...
			// end of new code in this section
		...
		private void Chess960Set(int[] Places)
		{

			HashSet<int> placesTaken = new HashSet<int>();
			int num = SetPosition(placesTaken);
			int rook1 = 0;
			int rook2 = 0;
			//set rooks
			...
			//keeps rooks from being next to each other
			HashSet<int> RookStabilizer = new HashSet<int>(placesTaken);
			RookStabilizer.Add(num - 1);
			RookStabilizer.Add(num + 1);

			num = SetPosition(RookStabilizer);
			//WhiteKingsRook;
			Places[1] = num;
			...
			//checks which rook pieces are where and places king
			if (rook2 < rook1)
			{
				num = random.Next(rook2 + 1, rook1);
				Places[2] = num;
				//WhiteKing;
				placesTaken.Add(num);
			}
			...
			int[] brownSpaces = { 0, 2, 4, 6 };

			bool whiteCheck = false;
			num = SetPosition(placesTaken);
			Places[3] = num;
			//WhiteQueensBishop;
			placesTaken.Add(num);
			for (int i = 0; i < whiteSpaces.Length; i++)
			{
				if (num == whiteSpaces[i])
				{
					whiteCheck = true;
				}
			}
			...
			if (whiteCheck == true)
			{
				HashSet<int> temp = new HashSet<int>(placesTaken);
				for (int i = 0; i < whiteSpaces.Length; i++)
				{
					temp.Add(whiteSpaces[i]);
				}
				num = SetPosition(temp);
				Places[4] = num;
				//WhiteKingsBishop;
				placesTaken.Add(num);
			}
			...
			{
				HashSet<int> temp = new HashSet<int>(placesTaken);
				for (int i = 0; i < brownSpaces.Length; i++)
				{
					temp.Add(brownSpaces[i]);
				}
				num = SetPosition(temp);
				Places[4] = num;
				//WhiteKingsBishop;
		...
		//modified code from: https://stackoverflow.com/questions/18484577/how-to-get-a-random-number-from-a-range-excluding-some-values#:~:text=Create%20an%20array%20containing%20all,at%20random%20from%20the%20array.&text=Put%20the%20allowed%20numbers%20into,of%20this%20array%20minus%20one.
		public int SetPosition(HashSet<int> numberSet)
		{
			var range = Enumerable.Range(0, 8).Where(i => !numberSet.Contains(i));

			var rand = new Random();
			int index = rand.Next(0, 8 - numberSet.Count);
			return range.ElementAt(index);
		}
		//end of code
		public static int[] Reverse(int[] whitePieces)
		{
			int[] reversedPositions = new int[whitePieces.Length];
			for (int i = 0; i < whitePieces.Length; i++)
			{
				switch (whitePieces[i])
				{
					case 0:
						reversedPositions[i] = 7;
						break;
					case 1:
						reversedPositions[i] = 6;
						break;
					case 2:
						reversedPositions[i] = 5;
						break;
					case 3:
						reversedPositions[i] = 4;
						break;
					case 4:
						reversedPositions[i] = 3;
						break;
					case 5:
						reversedPositions[i] = 2;
						break;
					case 6:
						reversedPositions[i] = 1;
						break;
					case 7:
						reversedPositions[i] = 0;
						break;

					default:
						reversedPositions[i] = whitePieces[i];
						break;
				}
			}
        ...
        public Piece GetWhiteType(int count, int position)
        {
            switch(count)
            {
                case 0:
                    return new Pieces.Rook(new Point(position, 0), true);
                case 1:
                    return new Pieces.Rook(new Point(position, 0), true);
                case 2:
                    return new Pieces.King(new Point(position, 0), true);
                case 3:
                    return new Pieces.Bishop(new Point(position, 0), true);
                case 4:
                    return new Pieces.Bishop(new Point(position, 0), true);
                case 5:
                    return new Pieces.Queen(new Point(position, 0), true);
                case 6:
                    return new Pieces.Knight(new Point(position, 0), true);
                case 7:
                    return new Pieces.Knight(new Point(position, 0), true);
                default:
                    return null;
		...
		public Piece GetBlackType(int count, int position)
		{
			switch (count)
			{
				case 0:
					return new Pieces.Rook(new Point(position, 7), false);
				case 1:
					return new Pieces.Rook(new Point(position, 7), false);
				case 2:
					return new Pieces.King(new Point(position, 7), false);
				case 3:
					return new Pieces.Bishop(new Point(position, 7), false);
				case 4:
					return new Pieces.Bishop(new Point(position, 7), false);
				case 5:
					return new Pieces.Queen(new Point(position, 7), false);
				case 6:
					return new Pieces.Knight(new Point(position, 7), false);
				case 7:
					return new Pieces.Knight(new Point(position, 7), false);
				default:
					return null;
